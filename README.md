...
_"`includeprev`" (неотлаженная)_. Добавляет файл с таким же названием из предыдущего загруженного архива, если он там существует. Если нет — ничего страшного не происходит.
...
"`setflag <flagname>`" или "`+f <flagname>`" устанавливает флаги текстур:
- "`handlecommonlines`" отвечает за обработку обычных линий (`action = 0`);
- "`handleactionlines`" отвечает за обработку линий с действием (`action ≠ 0`);
